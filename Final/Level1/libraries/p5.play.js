...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  //var console.log = pInstBind('console.log');
  ...
  */
  ...
  this.setDefaultCollider = function() {
    ...
    if(animations[currentAnimation] && (animations[currentAnimation].getWidth() !== 1 && animations[currentAnimation].getHeight() !== 1))
    {
      ...
      //quadTree.insert(this);
      this.colliderType = 'image';
      //console.log("IMAGE COLLIDER ADDED");
    }
      ...
      if(this.collider)
      {
        ...
        if(!mouseWasOver && this.mouseIsOver && this.onMouseOver !== undefined)
          if(typeof(this.onMouseOver) === 'function')
            ...
            console.log('Warning: onMouseOver should be a function');

        if(mouseWasOver && !this.mouseIsOver && this.onMouseOut !== undefined)
          if(typeof(this.onMouseOut) === 'function')
            this.onMouseOut.call(this, this);
          else
            console.log('Warning: onMouseOut should be a function');
        ...
        if(!mouseWasPressed && this.mouseIsPressed && this.onMousePressed !== undefined)
          if(typeof(this.onMousePressed) === 'function')
            ...
            console.log('Warning: onMousePressed should be a function');
        ...
        if(mouseWasPressed && !pInst.mouseIsPressed && !this.mouseIsPressed && this.onMouseReleased !== undefined)
          if(typeof(this.onMouseReleased) === 'function')
            ...
            console.log('Warning: onMouseReleased should be a function');
  ...
  this.setCollider = function(type, offsetX, offsetY, width, height) {
    ...
    if(type === 'rectangle' && arguments.length === 5) {
      ...
      if(arguments.length !== 4) {
        console.log('Warning: usage setCollider("circle", offsetX, offsetY, radius)');
      }
  ...
  */
  this.addToGroup = function(group) {
    if(group instanceof Array)
      ...
      console.log('addToGroup error: '+group+' is not a group');
  };
  ...
  */
  this.addAnimation = function(label)
  {
    ...
    {
      console.log('Sprite.addAnimation error: the first argument must be a label (String)');
      return -1;
    }
    else if(arguments.length < 2)
    {
      console.log('addAnimation error: you must specify a label and n frame images');
      return -1;
    }
  ...
  */
  this.changeAnimation = function(label) {
    if(!animations[label])
      console.log('changeAnimation error: no animation labeled '+label);
    else
    {
      currentAnimation = label;
  ...
  */
  this.overlapPixel = function(pointX, pointY) {
    ...
    {
      console.log('Error: overlapPixel doesn\'t work with scaled or rotated sprites yet');
      //offscreen console.loging to be implemented bleurch
      return false;
    }
  };
...
function Animation(pInst) {
  ...
  */
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    //console.log("sequence mode "+from+" -> "+to);

    //make sure the extensions are fine
    var ext1 = from.substring(from.length-4, from.length);
    if(ext1 !== '.png')
    {
      pInst.console.logln('Animation error: you need to use .png files (filename '+from+')');
      from = -1;
    }
    ...
    if(ext2 !== '.png')
    {
      pInst.console.logln('Animation error: you need to use .png files (filename '+to+')');
      to = -1;
    }
  ...
  {
    //console.log("Animation arbitrary mode");
  ...
  */
  ...
  this.draw = function(x, y, r) {
    ...
    if (this.visible)
    {
      ...
      {
        pInst.console.log('Warning undefined frame '+frame);
        //this.isActive = false;
      }
